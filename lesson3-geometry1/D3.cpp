...
struct Circle {
    ...
    ld r;
    bool contains(Point p) const;
    bool strictContains(Point p) const;
};
...
bool Circle::strictContains(Point p) const {
    return distance(p, o) - r < -EPS;
}

bool operator==(Circle c1, Circle c2) {
    return c1.o == c2.o && abs(c1.r - c2.r) <= EPS;
}
...
pair<pair<Point, Point>, int> operator^(Circle c1, Circle c2) {
    ...
    if (c1 == c2) {
        ...
        return {{c1.o, c1.o}, 0};
    } else if (c1.strictContains(c2.o)) {
    ...
    } else if (c2.strictContains(c1.o)) {
...
int main() {
    ...
    Circle c2{o2, r2};
    auto res = c1 ^ c2;
    Point p1 = res.first.first;
    Point p2 = res.first.second;
    cout << res.second << '\n';
    if (res.second == 1) {
        cout << p1.x << ' ' << p1.y << '\n';
    } else if (res.second == 2) {
        Point h = (Line{p1, p2} ^ Line{o1, o2}).first;
        cout << h.x << ' ' << h.y << '\n';
        cout << distance(o1, h) << ' ' << distance(p1, h) << '\n';
        cout << p1.x << ' ' << p1.y << '\n';
        cout << p2.x << ' ' << p2.y << '\n';
    }
}